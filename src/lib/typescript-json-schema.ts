...
function resolveRequiredFile(symbol: ts.Symbol, key: string, fileName: string, objectName: string): any {
    ...
    const requiredFilePath =  fileName // /^[.\/]+/.test(fileName)
        // ? fileName === "."
...
export class JsonSchemaGenerator {
    ...
    private parseCommentsIntoDefinition(symbol: ts.Symbol, definition: Definition, otherAnnotations: {}): void {
        ...
        jsdocs.forEach((doc) => {
            ...
            let [name, text] = [doc.name, doc.text as any as string];
            ...
            if (name.startsWith("TJS-")) {
                ...
                let match: string[] | RegExpExecArray | null = new RegExp(REGEX_TJS_JSDOC).exec(doc.text as any as string);
                if (match) {
                    name = match[1];
                    text = match[2];
                } else {
        ...
        if (valDecl?.initializer) {
            ...
            if ((<any>initial).expression) {
                ...
                try {
                    ...
                    eval("sandboxvar=" + initial.getText());
    ...
    private getTypeDefinition(
        ...
        if (asTypeAliasRef) {
            ...
            if (this.args.uniqueNames && reffedType) {
                ...
                const relativePath =  sourceFile.fileName // path.relative(process.cwd(), sourceFile.fileName);
...
function generateHashOfNode(node: ts.Node, relativePath: string): string {
        return md5.create().update(relativePath).update(node.pos.toString()).hex().substring(0, 8);
...
export function buildGenerator(
    ...
    if (diagnostics.length === 0) {
        ...
        program.getSourceFiles().forEach((sourceFile, _sourceFileIdx) => {
            const relativePath = sourceFile.fileName // = path.relative(workingDir, sourceFile.fileName);
...
export function programFromConfig(configFileName: string, onlyIncludeFiles?: string[]): ts.Program {
    ...
    const configParseResult = {} as any
...
export async function exec(program: ts.Program, filePattern: string, fullTypeName: string, args = getDefaultArgs()): Promise<Definition> {
    let onlyIncludeFiles: string[] | undefined = undefined;
    // if (REGEX_TSCONFIG_NAME.test(path.basename(filePattern))) {
    //     if (args.include && args.include.length > 0) {
    //         throw new Error("glob removed")
    //     }
